---
title: Como prever jogos de CS2 utilizando IA
excerpt: Uma LLM recebeu artigos e estatísticas de equipes do CS2 com a tarefa de prever quem seria o vencedor do primeiro Major de CS2.
...
![Simulação do "Pick'em" no CS2 (terminar essa descrição)](/cs2/cs2.jpg)
...
Posteriormente, refiz a análise, dessa vez usando um mecanismo melhor para obter notícias recentes de cada equipe:
...
Nas duas análises, as semifinais dos playoffs foi disputada essas 4 equipes:
...
E por fim, em ambas, **Spirit** foi declarada como campeã.
...
Estes são os dados e as estatísticas que fornecemos a LLM quando pedimos que ele previsse o vencedor de um jogo:
...
No final, o _prompt_ fornecido a LLM tem esse formato.

**Prompt de um jogo entre FURIA e Eternal Fire na rodada 1-0 na fase Challengers do PGL CS2 Major Championship:**

(O _prompt_ aqui foi traduzido para facilitar a compreensão. A interação com a LLM foi feita toda em inglês.)
...
Observe que usamos alguns conceitos de engenharia de prompts, como [Chain of thought](https://en.wikipedia.org/wiki/Prompt_engineering#Chain-of-thought), quando pedimos a LLM para "explicar passo a passo todo o seu raciocínio", e [Self-refine](https://en.wikipedia.org/wiki/Prompt_engineering#Self-refine), quando pedimos para "Depois de escolher seu vencedor, critique seu raciocínio e, em seguida, responda com sua resposta final".

## Respostas

Aqui estão algumas análises interessantes feitas pela LLM.

(As análise aqui foram traduzidas para facilitar a compreensão. A interação com a LLM foi feita toda em inglês.)
...
As notícias não podem ser inseridos na íntegra no LLM. Levaria muito tempo para ler todos eles, além de fazer com que minha conta do OpenAI fosse até o teto.

Portanto, antes de solicitar o vencedor da partida, enviamos cada artigo ao LLM para solicitar um resumo:
...
Para os artigos, usei duas abordagens diferentes:
