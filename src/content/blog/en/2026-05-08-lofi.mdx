...
Going back a little back in history: The MP3 format itself is widely used to store and transport audio files. It is very efficient to compress an audio file size while maintaing the quality.
...
This compression format states **each frame of an audio file stores exactly 1152 samples**. Now, there is another metric called **sampling rate**, which basically tell us how many times per second the analog audio signal is converted to 1's and 0's.
...
So if that doesn't work, what does? Well, we need to implement something called `PreciseTimer` that can respond in sub-millisecond range.

We calculate how many time it takes to broadcast the audio frame and wait the difference so it adds up to the `26ms` we need to respect:
...
I/O operations are generally expensive. For the radio, we are triggeting the next file read a few seconds _before_ the current track ends so it has enough time for the `mp3Parser` function to run. To also migigate any latency, we read the file in _async_ mode.
...
- [ ] Normalize the MP3 tracks in the server so they are all in 44.1kHz sampling rate
